...
app.get("/scrape", async (req, res) => {
  const partNumbers = ["M87686-601", "M07203-001"];
  ...
  try {
    ...
    for (let i = 0; i < partNumbers.length; i++) {
      ...
      // first check is exist value or not if exist then need to clear and set new value
      await page.evaluate(() => {
      ...
      const pageData = await page.evaluate(async () => {
        const tables = await Array.from(document.querySelectorAll("table"));
        const descriptions = [];
        if (tables.length > 0) {
